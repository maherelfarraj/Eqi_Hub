...
**How to apply:** Base a new tree and commit on the PR head, patch the plural ref endpoint with `force: false`, then confirm the branch via matching refs or `git ls-remote` before reporting that the PR was updated.