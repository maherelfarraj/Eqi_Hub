...
const prepare = (input) => { if (input.batch10_manifest_hash === "GENERATE") input.batch10_manifest_hash = buildBatch10Manifest(input.batch10_manifest); return input; };
...
test("produces a deterministic release manifest", () => { const a = evaluateDatasetRelease(prepare(fixture("release-synthetic-ready")), config); const b = evaluateDatasetRelease(prepare(fixture("release-synthetic-ready")), config); assert.equal(a.decision, "ready"); assert.equal(a.release_manifest.release_manifest_hash, b.release_manifest.release_manifest_hash); });
...
test("rejects insufficient split coverage", () => { const input = prepare(fixture("release-synthetic-ready")); input.batch10_manifest.rows = input.batch10_manifest.rows.filter((row) => row.split !== "golden"); input.batch10_manifest_hash = buildBatch10Manifest(input.batch10_manifest); assert.ok(evaluateDatasetRelease(input, config).rejection_codes.includes("coverage-insufficient")); });
test("rejects insufficient visibility", () => { const input = prepare(fixture("release-synthetic-ready")); input.batch10_manifest.rows.forEach((row) => row.content.keypoints.forEach((point) => { point.state = "outside-frame"; point.x = null; point.y = null; })); input.batch10_manifest_hash = buildBatch10Manifest(input.batch10_manifest); assert.ok(evaluateDatasetRelease(input, config).rejection_codes.includes("visibility-insufficient")); });
test("rejects missing Batch 10 lineage", () => { const input = prepare(fixture("release-synthetic-ready")); input.batch10_manifest.rows[0].window_lineage_ref = null; input.batch10_manifest_hash = buildBatch10Manifest(input.batch10_manifest); assert.ok(evaluateDatasetRelease(input, config).rejection_codes.includes("batch10-lineage-missing")); });
test("canonicalizes manifest object key order", () => { const input = prepare(fixture("release-synthetic-ready")); const manifest = input.batch10_manifest; input.batch10_manifest = Object.fromEntries(Object.entries(manifest).reverse()); assert.equal(buildBatch10Manifest(input.batch10_manifest), input.batch10_manifest_hash); });
test("rejects reordered Batch 10 rows", () => { const input = prepare(fixture("release-synthetic-ready")); input.batch10_manifest.rows.reverse(); input.batch10_manifest_hash = buildBatch10Manifest(input.batch10_manifest); assert.ok(evaluateDatasetRelease(input, config).rejection_codes.includes("batch10-lineage-missing")); });
