...
test("rejects thresholds that weaken quality or overfit gates", () => { const unsafe = structuredClone(config); unsafe.thresholds.minimum_pck = 0.01; unsafe.thresholds.maximum_mean_error = 1; unsafe.thresholds.maximum_validation_test_gap = 1; assert.ok(validateBatch13Config(unsafe).some((error) => error.includes("safe bounds"))); });
